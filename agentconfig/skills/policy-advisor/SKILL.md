---
name: osg-policy-advisor
description: Use when an osg sandbox returns policy_denied, mentions policy.local, or needs a narrow network policy proposal.
---

# osg Policy Advisor

When a request fails with `policy_denied`, do not treat the denial as final if
the user task still needs the request. Read `/etc/osg/skills/policy_advisor.md`,
